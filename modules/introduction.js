      ...
      pages: [
        ...
        <div class="inst-wrap">
          ...
          <div class="scroll-box" id="consent-scroll-zone">
            ...
            <p><strong>1. 同意の取得</strong><br>本実験にご協力いただきありがとうございます。始めに実験内容について、下までスクロールしながらご確認ください。</p>
            <p><strong>2. 研究目的と内容</strong><br>本研究は、画像とその順序の記憶について明らかにすることを目的としております。提示される画像について簡単な判断を行い、その後で画像の背景・順序について回答してもらいます。実験は説明を含めて25分程度で終了します。</p>
            <p><strong>3. 参加者の権利</strong><br>・本研究の参加は自由意志によるものであり、参加への同意はいつでも撤回することができます。<br>・同意を撤回しても、いかなる不利益を受けることはありません。<br>・途中で参加の同意を撤回し実験を中止する場合は、そのままウェブブラウザを閉じてください。そのデータは研究では使用いたしません。</p>
            <p><strong>4. 謝礼</strong><br>・謝礼はYahoo!クラウドソーシングを通して支払われます。<br>・実験終了後、最後の画面に6桁の確認番号（作業完了コード）が表示されます。必ず記録し、Yahoo!クラウドソーシングの画面に戻って入力してください。<br>・入力ミスや、定員への到達などによって謝礼が受け取れなかった場合の補償はいたしかねますので、あらかじめご了承ください。</p>
            <p><strong>5. 研究結果の使用およびプライバシーの保護</strong><br>・本研究で得られたデータは、研究の再現性の保証と研究分野のさらなる発展のために、インターネット上のサイトにおいて公開する可能性があります。<br>・本研究では匿名でデータを収集するため、公開されるのは実験内で選択された回答と年齢、性別のみで、個人が特定されることはありません。<br>・また、研究の成果は学術論文や学会発表などを通じて公表される可能性がありますが、データは全体の傾向として統計的に処理・分析されるため、個人が特定されることはありませんのでご安心ください。</p>
            <p><strong>6. 倫理委員会</strong><br>この実験は、九州大学大学院人間環境学研究院の研究倫理委員会の承認を得て実施しています（申請番号：${config.investigator.ethics_approval_number}）。</p>
          </div>
      ...
      pages: [
        ...
        <div class="inst-wrap">
          ...
          <ul>
            ...
            <li><strong>画像の背景を思い出す課題</strong>：<br>表示されていた（=F）と答えた画像について、それが「どの枠色」または「どの位置」で提示されていたかを2択で回答します。</li>
          </ul>
          <p>練習課題では、ダミー（非表示画像）を含みません。</p>
          
          <div class="img-pair">
            <img src="./img/inst_recongnition.png" style="max-width: 45%; height: auto; border: 2px solid #000;" alt="画像の有無を思い出す課題の説明">
            <img src="./img/inst_source.png" style="max-width: 45%; height: auto; border: 2px solid #000;" alt="画像の背景を思い出す課題の説明">
          </div>
        </div>
        `,
        // 共通ページ3：ステップ4（inst_order.png を追加）
        `
        <div class="inst-wrap">
          ...
          <img src="./img/inst_order.png" class="inst-img" alt="画像の順序を思い出す課題の説明">
        </div>
        `
      ],
      show_clickable_nav: true,
